 ...
 */
function unsleepable_scripts() {
	wp_enqueue_style( 'unsleepable', get_stylesheet_uri() );

	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}
}
add_action( 'wp_enqueue_scripts', 'unsleepable_scripts' );

/**
 * Filters wp_title to print a neat <title> tag based on what is being viewed.
 *
 * @since Unsleepable 1.0
 */
function unsleepable_wp_title( $title, $sep ) {
	global $page, $paged;
	...
	if ( $paged >= 2 || $page >= 2 )
		$title .= " $sep " . sprintf( __( 'Page %s', 'unsleepable' ), max( $paged, $page ) );

	return $title;
}
add_filter( 'wp_title', 'unsleepable_wp_title', 10, 2 );
